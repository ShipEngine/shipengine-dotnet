...
namespace ShipEngineTest
{
    public class GetRatesFromShipmentTest
    {
        ...
        public async void ValidCreateLabelFromShipmentDetailsTest()
        {
            ...
            var shipEngine = new ShipEngine("TEST_ycvJAgX6tLB1Awm9WGJmD8mpZ8wXiQ20WhqFowCk32s");

            var ratesParams = new RatesParams()
            {
                Shipment = new Shipment()
                {
            ...
            var ratesResponse = await shipEngine.GetRatesWithShipmentDetails(ratesParams);

            Console.WriteLine(ratesResponse);
